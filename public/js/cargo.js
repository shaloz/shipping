    ...
    $.post("/clients/searchQ", { data: value }, function(dataFound) {
      ...
      if (dataFound.length !== 0) {
        ...
        dataFound.forEach(function(result) {
          ...
          client_table += "<td>";
          client_table +=
            '<button class="btn btn-outline-info btn-sm newCargoBtn" id="' +
            result._id +
            '"><i class="fas fa-plus"></i> New Container</button>';
          client_table += "</td>";
          client_table += "</tr>";
    ...
    $.get("/clientsAjax", function(data) {
      ...
      data.forEach(function(result) {
        ...
        client_table +=
          '<button class="btn btn-outline-info btn-sm newCargoBtn" id="' +
          result._id +
          '"><i class="fas fa-plus"></i> New Cargo</button>';
        client_table += "</td>";
        client_table += "</tr>";
...
$(document).ready(function() {
    ...
    $.get(
      ...
      function(data) {
        ...
        if (data.Results[8].Value !== null) {
          ...
          if (data.Results[12].Value !== null) {
            $("#cardetails").val(
              data.Results[9].Value +
                " " +
                data.Results[6].Value +
                " " +
                data.Results[8].Value +
                " " +
                data.Results[12].Value
            );
    ...
    $.get("/cargo/" + value + "/fetchCargo_perclient", function(result) {
      ...
      if (result.length !== 0) {
        result.forEach(function(content) {
          ...
          html += "<td>";
          html +=
            '<button class="btn btn-outline-dark btn-sm openAddMoreCars" id="' +
            content._id +
            '" data-toggle="modal" data-target="#addMoreCarsModal"><i class="fas fa-plus"></i> Add more cars</button> ';
          html +=
            '<a class="btn btn-outline-info btn-sm openEditModal" href="cargo/' +
            content._id +
  ...
  $(document).on("click", ".savechanges_personaleffect", function() {
    ...
    //send data to server
    $.post(
      "/cargo/" + cargo_id + "/edit_personaleffect",
      { data: data },
      function(result) {
        console.log(result);
