...
router.get("/cargo/displaynewroro", function(req, res) {
  if (req.user) {
    ...
    async.parallel(
      ...
      (err, results) => {
        var clientsData = results[0];
        return res.render("main/newRoro", {
          title: "Oldsailor Ocean Shipping LLC || New Cargo",
          clientsData: clientsData,
          layout: false
        });
      }
    ...
    async.parallel(
      [
        function(callback) {
          ...
          Cargos.findByIdAndUpdate(
            req.body.data.cargo_id,
            { personal_effect: req.body.data.personal_effect },
        ...
        function(callback) {
          ...
          Cargos.findOne({ _id: req.body.data.cargo_id }, function(
            err,
            cargos
          ) {
            if (req.body.data.vin !== "") {
...
router.post("/roro/:id/addmorecars", function(req, res) {
  ...
  //update Cars Array
  //push new cars into the array
  Cargos.findOne({ _id: req.params.id }, function(err, cargos) {
...
router.get("/roro/:value/fetchCargo_perclient", function(req, res) {
  if ("Display cargo per client" === req.params.value) {
    ...
    // console.log();
    Cargos.find({ Client: req.params.value })
      .populate("Client")
      .exec(function(err, foundData) {
...
router.delete("/roro/:id/delete", function(req, res) {
  Cargos.findByIdAndRemove(req.params.id, function(err) {
    ...
    else return res.send(req.params.id);
  });
});
